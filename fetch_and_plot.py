...
NUM_STRIKES     = 8                 # ATM + 12 OTM  (indices 0..12)
...
def cache_path(symbol: str, fetch_date: str) -> Path:
    ...
    return DATA_DIR / f"{safe}_{fetch_date}.json"
...
def generate_dashboard(
    ...
    def build_atm_iv_spot_traces(ce_data, pe_data, spot_df):
        ...
        ce_iv_trace = {
            "x": ts_to_list(ce_atm["iv"].dropna()), "y": vals_to_list(ce_atm["iv"].dropna()),
            "name": f"ATM CE IV ({ce_atm['strike']})", "type": "scatter", "mode": "lines",
            "line": {"color": "#00d9ff", "width": 2},
            "yaxis": "y",
        }
        pe_iv_trace = {
            "x": ts_to_list(pe_atm["iv"].dropna()), "y": vals_to_list(pe_atm["iv"].dropna()),
            "name": f"ATM PE IV ({pe_atm['strike']})", "type": "scatter", "mode": "lines",
            "line": {"color": "#ff6b6b", "width": 2},
            "yaxis": "y",
        }
        return [ce_iv_trace, pe_iv_trace, spot_trace]
...
def main():
    ...
    print(f"\U0001f4ca CE strikes  : {ATM_STRIKE} \u2192 {ATM_STRIKE + (NUM_STRIKES-1)*STRIKE_STEP}  (OTM calls)")
    print(f"\U0001f4ca PE strikes  : {ATM_STRIKE} \u2192 {ATM_STRIKE - (NUM_STRIKES-1)*STRIKE_STEP}  (OTM puts)")
    print(f"\u23f1  T method    : Trading minutes only  (252d \u00d7 {MINUTES_PER_DAY}min/d)")
    print(f"\U0001f4b9 Risk-free r : {RISK_FREE_RATE*100:.1f}%")

    fyers = get_fyers_client()

    # ── Spot ──────────────────────────────────────────────────────────────────
    print(f"\n\u2500\u2500 Fetching Spot \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    spot_df = fetch_or_load(fyers, SPOT_SYMBOL, fetch_date)
    ...
    for item in ce_syms:
        ...
        print(f"    IV range  [{item['strike']} CE]: {iv.dropna().min():.1f}% – {iv.dropna().max():.1f}%")
    ...
    for item in pe_syms:
        ...
        print(f"    IV range  [{item['strike']} PE]: {iv.dropna().min():.1f}% – {iv.dropna().max():.1f}%")
